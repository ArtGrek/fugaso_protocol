...
use fugaso_math::protocol_mega_thunder::{
	...
	COIN, JACKPOT, MULTI, COIN_SYMBOL, COIN_COLUMN, GRAND_JACKPOT, 
};
...
fn test_convert() {
    ...
    convert("d881edc87b9a4faf8a8edd304943f635.json");
}
				...
				if grand_lightning_in.command == CommandEnum::Play {
					...
					#[cfg(any())]
					{
						...
						let response: SpinData<MegaThunderLinkInfo, ReSpinInfo> = {
							...
							let holds = vec![0];
							let grid0 = vec![];
							let grid = convert_board(&bonus.board);
                            let mut gains = vec![];
							//special
								...
								if mults.iter().flatten().any(|&v| v > 0) && lifts.iter().flatten().any(|&v| v > 0) {
									...
									gains.push(
										Gain {
											symbol: COIN_SYMBOL, 
											count: coins_count, 
											amount: coins_win_amount as i64, 
											line_num: 0, 
											multi: 1, 
											..Default::default()
										}
					...
					#[cfg(any())]
					{
						...
						let response: SpinData<MegaThunderLinkInfo, ReSpinInfo> = {
							...
							let (grid, overlay) = if let Some(board) = context.spins.original_board {
								let grid = convert_board(&board);
								let overlay = Some(convert_board(&context.spins.board));
								(grid, overlay)
							} else {
								let grid = convert_board(&context.spins.board);
								let overlay = None;
								(grid, overlay)
							};
								...
								context.spins.bs_values.iter().enumerate().for_each(|(col_num, col)| {
									col.iter().enumerate().for_each(|(row_num, &v)| {
										if context.spins.board[col_num][row_num] == MULTI {
											...
											lifts[col_num][row_num] = v as i32;
											lifts_new.push(LiftItem {
												p: (col_num, row_num),
												m: v as i32,
...
fn convert_board(board: &Vec<Vec<i64>>) -> Vec<Vec<char>> {
    board.iter().enumerate().map(|(_col_num, col)| {
		col.iter().enumerate().map(|(_row_num, row)| {
